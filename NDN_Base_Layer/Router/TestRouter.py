...
class Data:
    ...
    def on_data(self, sizes, route_id, data, tables, result_save, thread_lock):
        """
        data = {'type': 'data', 'consumer_ID': 0, 'route_ID': 0, 'content_name': 'r0/0', 'content_data': '',
                'data_hop': 0, 'run_start_time': 0.0, 'path': ''}
        ...
        """
        content_store = CS()
        pending_interest_table = PIT()
        forward_information_base = FIB()
...
class Interest:
    ...
    @staticmethod
    def send_interest(pit, fib, outfaces, route_id, interest):
        """
        outfaces = [outface, ...]
        """
        # Send interest
    ...
    def on_interest(self, route_id, interest, tables, sizes, result_save, thread_lock):
        content_store = CS()
        pending_interest_table = PIT()
        forward_information_base = FIB()
        da = Data()
        ...
        if search_pit_ack:
            ...
            outfaces = forwarding.forward_interest(fib, network, route_id, interest)
            if len(outfaces) > 0:
